    ...
    try
      ...
      with cxtbBrgKeluar.DataController do begin
        for i := 0 to RecordCount - 1 do begin
          ...
          qd.FieldByName('kode_brg').AsString := GetKodeBrg(Values[i, cxColDeskripsi.Index]);
      ...
      dm.zConn.ExecuteDirect(
        Format('UPDATE tbl_trskeluar_head SET f_posting = 1 WHERE id = %d',[ID])
      );
      dm.zConn.Commit;
      qh.Close;
      qd.Close;
      ...
      if Assigned(Self.FormInduk) then begin
        try
          ...
          (Self.FormInduk as TfrmLstBarangKeluar).zqrBarangKeluar.Last;
        except
        end;
      end;
