      ...
      for i := 0 to RecordCount -1 do begin
        ...
        if Self.Jenis = 'T' then begin
          ...
          qbrg := OpenRS('SELECT * FROM tbl_barang_det WHERE id_brg = %s AND id_gdg = %s',
            [Values[i, cxColDeskripsi.Index], Values[i, cxColGudangAsal.Index]]);
          ...
          qbrg := OpenRS('SELECT * FROM tbl_barang_det WHERE id_brg = %s AND id_gdg = %s',
            [Values[i, cxColDeskripsi.Index], Values[i, cxColGudangTujuan.Index]]);
          if qbrg.IsEmpty then begin
            qbrg.Insert;
            qbrg.FieldByName('id_brg').AsInteger := Values[i, cxColDeskripsi.Index];
            qbrg.FieldByName('kode_brg').AsString := Values[i, cxColKode.index];
...
begin
  ...
  if AItemIndex = cxColKode.Index then begin
    try
      ...
      with cxTblTransBarang.DataController do begin
        ...
        Values[ARecordIndex, cxColKodeBrg2.Index] := z.FieldByName('kode').AsString;
        Values[ARecordIndex, cxColSatuan.Index] := z.FieldByName('satuan').AsString ;
        Values[ARecordIndex, cxColIdSatuan.Index] := z.FieldByName('id_satuan').AsInteger ;
      end;
      z.Close ;
  ...
  else if AItemIndex = cxColDeskripsi.Index then begin
    try
      cxTblTransBarang.BeginUpdate;
      z := OpenRS('SELECT a.id_satuan,a.kode,b.satuan FROM tbl_barang a join tbl_satuan b on a.id_satuan=b.id where a.id =%s',
      ...
      with cxTblTransBarang.DataController do begin
        Values[ARecordIndex, cxColKodeBrg2.Index] := z.FieldByName('kode').AsString;
        Values[ARecordIndex, cxColKode.Index] := ADataController.Values[ARecordIndex, AItemIndex];
        Values[ARecordIndex, cxColSatuan.Index] := z.FieldByName('satuan').AsString ;
        Values[ARecordIndex, cxColIdSatuan.Index] := z.FieldByName('id_satuan').AsInteger ;
      end;
      z.Close ;
