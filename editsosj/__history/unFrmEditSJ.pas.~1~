...
type
  ...
  private
    ...
    function CheckSOComplete(id: Integer): boolean;
  public
    { Public declarations }
  end;

var
  frmEditSJ: TfrmInputSuratJalan;
...
procedure TfrmEditSJ.btnSimpan2Click(Sender: TObject);
var
  ...
  with cxtbSJTanpaSO.DataController do begin
    ...
    try
      ...
      if Self.Jenis = 'T' then begin
        ...
        dm.zConn.ExecuteDirect(Format('DELETE FROM tbl_history WHERE id_ref = %s AND LEFT(no_bukti,2) = ''SJ''',
          [Self.EditKey])
        );
      end;
      ...
      with cxtbSJTanpaSO.DataController do begin
        for i := 0 to RecordCount - 1 do begin
          ...
          with qhs do begin
            ...
            FieldByName('kode_brg').AsString := Values[i, cxColKodeBrg3.Index];
            FieldByName('id_brg').AsInteger := Values[i, cxColKodeBrg2.Index];
            FieldByName('qty').AsFloat := Values[i, cxColQty2.Index];
            FieldByName('tipe').AsString := 'o';
            FieldByName('id_satuan').AsInteger := Values[i, cxColIdSatuan2.Index];
            FieldByname('id_gdg').AsInteger := Values[i, cxColGudang2.Index];
          ...
          qbrg := OpenRS('SELECT * FROM tbl_barang_det WHERE id_brg = %s AND id_gdg = %s',
            [Values[i, cxColKodeBrg2.Index], Values[i, cxColGudang2.Index]]);
...
procedure TfrmEditSJ.btnSimpanClick(Sender: TObject);
var
  q, qh, qd, qSatuan, qGdg, qhs, qbrg, qjd : TZQuery;
  sNoBukti, sNoFaktur, sNoJ : string;
  i, id, id_akun : integer;
      ...
      with cxtbSJ.DataController do begin
        for i := 0 to RecordCount - 1 do begin
          ...
          with qhs do begin
            ...
            FieldByName('id_brg').AsInteger := Values[i, cxColIdBrg.Index];
          ...
          qbrg := OpenRS('SELECT * FROM tbl_barang_det WHERE id_brg = %s AND id_gdg = %s',
            [Values[i, cxColIdBrg.Index], Values[i, cxColGudang.Index]]);
          if qbrg.IsEmpty then begin
            qbrg.Insert;
            qbrg.FieldByName('id_brg').AsInteger := qd.FieldByName('id_brg').AsInteger;
            qbrg.FieldByName('kode_brg').AsString := qd.FieldByName('kode_brg').AsString;
...
procedure TfrmEditSJ.Button1Click(Sender: TObject);
begin
  inherited;
  cxgSJTanpaSO.Visible := True;
end;

procedure TfrmEditSJ.btnPilihSOClick(Sender: TObject);
...
procedure TfrmEditSJ.cxChkPPNClick(Sender: TObject);
var
...
procedure TfrmEditSJ.cxChkSJTanpaSOClick(Sender: TObject);
begin
  inherited;
...
procedure TfrmEditSJ.cxColNoGetDisplayText(
...
procedure TfrmEditSJ.cxColNoSOPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
var
  f: TfrmPilihSO;
  i: integer;
...
procedure TfrmEditSJ.cxlCustomerPropertiesChange(Sender: TObject);
var
  q: TZQuery;
begin
  inherited;
...
procedure TfrmEditSJ.cxsDiskonPropertiesChange(Sender: TObject);
begin
  inherited;
  HitungTotal;
end;

procedure TfrmEditSJ.cxtbBarangMasukDataControllerBeforePost(
  ADataController: TcxCustomDataController);
var
  i,j,k: integer;
  v: variant;
...
procedure TfrmEditSJ.cxtbBarangMasukDataControllerRecordChanged(
...
procedure TfrmEditSJ.cxtbSJDataControllerAfterDelete(
  ADataController: TcxCustomDataController);
begin
  inherited;
  try
...
procedure TfrmEditSJ.cxtbSJDataControllerBeforePost(
  ADataController: TcxCustomDataController);
var
  i,j,k: integer;
  v: variant;
...
procedure TfrmEditSJ.cxtbSJDataControllerNewRecord(
  ADataController: TcxCustomDataController; ARecordIndex: Integer);
begin
  inherited;
...
procedure TfrmEditSJ.cxtbSJDataControllerRecordChanged(
  ADataController: TcxCustomDataController; ARecordIndex, AItemIndex: Integer);
var
  q: TZQuery;
  t, t1, t2, t3 : Real;
...
procedure TfrmEditSJ.cxtbSJTanpaSODataControllerAfterPost(
  ADataController: TcxCustomDataController);
begin
  inherited;
  try
...
procedure TfrmEditSJ.cxtbSJTanpaSODataControllerBeforePost(
  ADataController: TcxCustomDataController);
var
  i,j,k: integer;
  v: variant;
...
procedure TfrmEditSJ.cxtbSJTanpaSODataControllerNewRecord(
  ADataController: TcxCustomDataController; ARecordIndex: Integer);
begin
  inherited;
...
procedure TfrmEditSJ.cxtbSJTanpaSODataControllerRecordChanged(
  ADataController: TcxCustomDataController; ARecordIndex, AItemIndex: Integer);
var
  q: TZQuery;
  t, t1, t2, t3, sa : Real;
  ...
  if AItemIndex = cxColGudang2.Index then begin
    try
      ...
      if VarIsNull(ADataController.Values[ARecordIndex, cxColDeskripsi2.Index]) then begin
        ADataController.Values[ARecordIndex, cxColGudang2.Index] := '';
        Abort;
      end;
      //cxtbSJTanpaSO.EndUpdate;
    except
...
procedure TfrmEditSJ.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TfrmEditSJ.FormShow(Sender: TObject);
  var
   q,z : TZQuery;
   i, nomer: integer;
   sNoTrs : string;
...
procedure TfrmEditSJ.HitungTotal;
var
  i: integer;
...
begin
  try
    if cxChkSJTanpaSO.Checked then begin
      ...
      if cxsDPP.Value = 0 then
        cxsHargaTotal.Value := cxsStlhDiskon.Value
      else
...
procedure TfrmEditSJ.Posting(IDSJ: Integer);
var
...
function TfrmEditSJ.CheckSOComplete(id: integer): boolean;
var
  qsj, q: TZQuery;
  f: Boolean;
  a,b: Extended;
  i: integer;
begin
  if not cxChkSJTanpaSO.Checked then begin
    qsj := OpenRS('SELECT * FROM tbl_sj_det WHERE id_ref = %d',[id]);
    with qsj do begin
